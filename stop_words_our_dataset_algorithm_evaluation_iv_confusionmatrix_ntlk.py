# -*- coding: utf-8 -*-
"""Stop Words - Our Dataset - Algorithm Evaluation - IV - ConfusionMatrix -  NTLK.ipynb
...
setenceswithstemmingtrain = applystemmer(basetreinamento)
setenceswithstemmingtest = applystemmer(baseteste)
#print(setenceswithstemmingtrain)
...
sentencestrain = searchwords(setenceswithstemmingtrain)
sentencestest= searchwords(setenceswithstemmingtest)

def searchfrequency(sentences):
  sentences = nltk.FreDist(sentences)
  return sentences

freqtrain = searchfrequency(sentencestrain)
freqtest = searchfrequency(sentencestest)
...
uniquewordtrain = uniquewordsearch(freqtrain)
uniquewordtest = uniquewordsearch(freqtest)
print(uniqueword)

def extratorword(document):
  doc = set(document)
  characteristic = {}
  for words in uniqueword:
    characteristic['%s' % word] = (word in doc)
  return characteristic

characteristicsentences = extratorword(['am', 'nov', 'dia'])
#print(characteristicsentences)

dataset_complete_train = nltk.classify.apply_features(extratorword, setenceswithstemmingtrain)
dataset_complete_test = nltk.classify.apply_features(extratorword, setenceswithstemmingtest)
#print(dataset_complete)

classifier = nltk.NaiveBayesClassifier.train(dataset_complete_train)

print(classifier.labels())
print(classifier.show_most_informative_features(5))

print(nltk.classify.accuracy(classifier, dataset_complete_test))

error = []
for (sentence, emotion) in dataset_complete_test:
  print(sentence)
  print(emotion)
  results = classifier.classify(sentence)
  if results != emotion:
    error.append((emotion,results,sentence))
    print(results)

#for (emotion, results, sentence) in error:
 # prin(emotion, results, sentence)
...
for (word) in test.split():
  withstem = [p for p in word.split()]
  teststemming.append(str(stemmer.stem(withstem[0])))

new = extratorword(teststemming)
print(classifier.classify(new))
distri = classifir.prob_classify(new)
for class_ in distri.samples():
  print("%s: %f" % (class_, distri.prob(class_)))